...
OrbVis is also working towards inclusion as a built-in Checkmk package
(targeted for Checkmk 2.6.0). For that path, contributions to the
`upstream-checkmk/` subtree may need to be re-licensed under Checkmk
GmbH's contributor terms. We will reach out before any such transfer; in
the meantime, the public repo stays plain GPL-2.0-only.
