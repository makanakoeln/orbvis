...
# Auto-generated by install.sh
#
# Alias-ordering caveat: Apache picks the longest matching prefix among Alias
# directives, but some older 2.4 versions on RHEL fall back to first-match
# when prefixes share a common root. Spell the specific aliases out before
# the catch-all htdocs alias so both behaviours land on the right target.
