...
from cmk.orbvis_backend.core._env import load_env_file
...
class Settings(BaseModel):
    ...
    maps_dir: str = Field(default_factory=lambda: _env_str("MAPS_DIR", "./maps"))
    ...
    flow_map_max_services_per_host: int = Field(
        default_factory=lambda: _env_int("FLOW_MAP_MAX_SERVICES_PER_HOST", 50)
    )
    flow_map_topology_cache_ttl: float = Field(
        default_factory=lambda: _env_float("FLOW_MAP_TOPOLOGY_CACHE_TTL", 20.0)
    )
    ...
    flow_map_top_affected_hosts: int = Field(
        default_factory=lambda: _env_int("FLOW_MAP_TOP_AFFECTED_HOSTS", 25)
    )

    flow_map_bulk_service_chunk_size: int = Field(
        default_factory=lambda: _env_int("FLOW_MAP_BULK_SERVICE_CHUNK_SIZE", 5), ge=1
    )
    ...
    flow_map_bulk_max_concurrent_chunks: int = Field(
        default_factory=lambda: _env_int("FLOW_MAP_BULK_MAX_CONCURRENT_CHUNKS", 8),
        ge=1,
        le=64,
    )
    ...
    # map editor). Without it a multi-million-host site would stream every
