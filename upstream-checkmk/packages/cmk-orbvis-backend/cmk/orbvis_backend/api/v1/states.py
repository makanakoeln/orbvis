...
from cmk.orbvis_backend.api.v1.connections import auth_user_scope, build_topology_response
from cmk.orbvis_backend.api.v1.deps import can_view_map as _can_view_map
from cmk.orbvis_backend.api.v1.deps import can_view_map_by_name as _can_view_map_by_name
from cmk.orbvis_backend.api.v1.deps import get_current_user, resolve_auth_user
from cmk.orbvis_backend.api.v1.types import MapName
from cmk.orbvis_backend.connections.base import ServiceMatchRow, ServiceRow
from cmk.orbvis_backend.core.config import settings
from cmk.orbvis_backend.core.database import get_db
from cmk.orbvis_backend.core.ratelimit import ws_connect_limiter
from cmk.orbvis_backend.core.sse import Subscriber, manager
from cmk.orbvis_backend.integrations.checkmk import resolve_folder_scope
from cmk.orbvis_backend.models.user import User
from cmk.orbvis_backend.schemas.map import MapConfig, FlowView, FolderTreeView, RadarView
...
from cmk.orbvis_backend.schemas.topology import TopologyNode
from cmk.orbvis_backend.services import map_service, settings_service, state_service
...
async def _fetch_topology_for_user(
    ...
    try:
        ...
        logger.warning("topology fetch failed for map '%s'", cfg.name, exc_info=True)
        return None
...
async def _broadcast_loop(map_name: str) -> None:
    ...
    """
    logger.debug("Broadcast loop started for map '%s'", map_name)
    try:
        while manager.get_subscriber_count(map_name) > 0:
            cfg = map_service.get_map(map_name)
            tick_start = asyncio.get_event_loop().time()
            user_count = 0
            if cfg is not None:
                if settings.checkmk_omd_root:
                    grouped = manager.get_subscribers_grouped(map_name)
                    ...
                    for group_key, subs in grouped.items():
                        ...
                        can_view = (
                            (lambda n, u=auth_user: _can_view_map_by_name(u, n))
                        ...
                        ft_delta = (
                            state_service.compute_folder_tree_delta(
                                map_name, group_key, states.folder_tree
                            )
                        ...
                        if is_full or to_send or removed_ids or timing or ft_changed or ds_changed:
                            msg = _build_states_msg(
                                map_name, states, to_send, removed_ids, is_full, timing, ft_delta
                            )
                            manager.push(map_name, subs, msg)
                    ...
                    to_send, removed_ids, is_full, timing = state_service.compute_states_delta(
                        map_name, None, states.states
                    )
                    ft_delta = (
                        state_service.compute_folder_tree_delta(
                            map_name, None, states.folder_tree
                        )
                        if cfg.view.type == "foldertree"
                        else None
                    )
                    ft_changed = ft_delta is not None and (ft_delta.full or bool(ft_delta.changed))
                    subs = list(manager.get_subscribers_grouped(map_name).get(None, []))
            ...
            logger.debug(
                "Broadcast tick map=%s user_groups=%d elapsed=%.0fms",
                map_name,
                user_count,
                elapsed_ms,
            )
        ...
        state_service.drop_topology_snapshot(map_name)
        state_service.drop_states_snapshot(map_name)
        state_service.drop_map_snapshots(_dead_sites_snapshots, map_name)
        logger.debug("Broadcast loop stopped for map '%s'", map_name)
...
async def get_map_states(
    ...
    states = await state_service.get_map_states(
        cfg,
        auth_user=auth_user,
        can_view_map=lambda n: _can_view_map(current_user, n),
...
async def get_folder_host_services(
    ...
    if not _can_view_map(current_user, name):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="No map view permission"
        )
    if not isinstance(cfg.view, FolderTreeView):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Not a foldertree map"
        )
    connection = state_service.get_connection(cfg.connection_id)
    if connection is None:
...
async def sse_map_states(
    ...
    if not _can_view_map(user, name):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="No map view permission"
        )
    ...
    # group reconverges (mirrors the Flow Map's force_full-on-join).
