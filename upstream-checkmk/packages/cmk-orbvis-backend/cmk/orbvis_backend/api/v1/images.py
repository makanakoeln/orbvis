...
from cmk.orbvis_backend.core.image_security import ICON_MIME_TYPES, ICON_SUFFIXES, is_valid_image
...
# Stems are restricted to the same safe charset as map names — uploads with
...
def _images_dir() -> Path:
    d = Path(settings.maps_dir).parent / "images"
...
def _find_image_usage(name: str) -> list[ImageUsageEntry]:
    """Return all maps that reference *name* as object icon or background."""
    usage: list[ImageUsageEntry] = []
    for map in map_service.list_maps():
        cfg = map_service.get_map(map.name)
        if cfg is None:
            continue
        object_ids: list[str] = []
...
async def upload_image(
    ...
    if _is_builtin(filename):
        ...
        # a builtin icon is referenced across maps and silently replacing
        # it would change every map at once.
        raise HTTPException(
