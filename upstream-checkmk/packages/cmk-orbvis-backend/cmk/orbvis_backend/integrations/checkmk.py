...
def check_map_permission(username: str, map_name: str, action: str) -> bool:
    """Return True if the Checkmk user may view or edit an OrbVis map.
    ...
    *action* must be ``'view'`` or ``'edit'``.
    ...
    if action == "view":
        return _has_permission(user_data, role_config, "orbvis.view_all") or _has_permission(
            user_data, role_config, f"orbvis.view_{map_name}"
        )
    if action == "edit":
        return _has_permission(user_data, role_config, "orbvis.edit_all") or _has_permission(
            user_data, role_config, f"orbvis.edit_{map_name}"
        )
    return False
...
def check_create_permission(username: str) -> bool:
    """Return True if the Checkmk user may create or delete maps.

    Mirrors the WATO declaration: ``orbvis.edit_all`` grants creating, modifying
    and deleting maps. Per-map ``orbvis.edit_<map>`` does not apply here —
    creating a map is not scoped to an existing map name.
    Always returns False when CHECKMK_OMD_ROOT is not configured.
    """
    if not settings.checkmk_omd_root:
        return False
