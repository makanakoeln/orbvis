...
from cmk.orbvis_backend.connections.base import BI_INT_TO_STATE, FolderTreeData, GeoHost, ServiceRow
...
from cmk.orbvis_backend.schemas.presentation import DataElement, PresentationView, ShapeElement
from cmk.orbvis_backend.schemas.state import (
    FolderHostService,
    FolderTreeDelta,
    FolderTreeNode,
    FolderTreeNodePatch,
...
from cmk.orbvis_backend.schemas.topology import (
    ServiceTiming,
    TopologyDelta,
    TopologyNode,
    TopologyTiming,
...
if TYPE_CHECKING:
    from cmk.orbvis_backend.connections.base import ConnectionBase
...
async def get_connection_objects(
    ...
    groups (mirrors :func:`get_map_states`).
    """
...
async def get_map_states(
    cfg: MapConfig,
    auth_user: str | None = None,
    can_view_map: Callable[[str], bool] | None = None,
    ...
    async def _run() -> MapStates:
        ...
        if (
            ...
            async with connection.with_auth_user(auth_user):
                return await _execute_map_states(
                    cfg, connection, auth_user=auth_user, can_view_map=can_view_map
                )
        return await _execute_map_states(
            cfg, connection, auth_user=auth_user, can_view_map=can_view_map
        )
...
async def inflate_auto_objects(cfg: MapConfig, connection: ConnectionBase) -> list[MapObject]:
    ...
    if view.auto_source is not None:
        ...
        try:
            ...
            logger.warning("Failed to fetch geo hosts for map %s", cfg.name, exc_info=True)
...
async def _states_for_objects(
    ...
    *,
    ...
    groups: dict[str, list[MapObject]] = {}
    for obj in objects:
        cid = obj.connection_id or default_connection_id
        groups.setdefault(cid, []).append(obj)

    async def _run(cid: str, group_objs: list[MapObject]) -> dict[str, ObjectState]:
...
@dataclass
class _MapObjectBins:
    """Map objects split by the kind of state query each needs."""

    hosts_soft: list[MapObject] = field(default_factory=list)
    hosts_hard: list[MapObject] = field(default_factory=list)
    svcs_soft: list[MapObject] = field(default_factory=list)
    svcs_hard: list[MapObject] = field(default_factory=list)
    lines: list[MapObject] = field(default_factory=list)
    map_objects: list[MapObject] = field(default_factory=list)
    aggregation_objs: list[MapObject] = field(default_factory=list)
    others: list[MapObject] = field(default_factory=list)


def _bin_map_objects(objects: list[MapObject]) -> _MapObjectBins:
...
async def _resolve_host_states(
    connection: ConnectionBase,
    hosts_soft: list[MapObject],
    hosts_hard: list[MapObject],
    auth_user: str | None,
...
async def _resolve_service_states(
    connection: ConnectionBase,
    svcs_soft: list[MapObject],
    svcs_hard: list[MapObject],
    auth_user: str | None,
    results: dict[str, ObjectState],
) -> None:
...
async def _resolve_aggregation_states(
    connection: ConnectionBase,
    aggregation_objs: list[MapObject],
    results: dict[str, ObjectState],
) -> None:
    ...
    # Tree-fetch deduplicated by (aggregation_id, depth) — multiple map
...
async def _resolve_map_link_states(
    ...
    auth_user: str | None,
    can_view_map: Callable[[str], bool] | None,
    ...
    if not map_objects:
        return
    from cmk.orbvis_backend.services import map_service
    ...
    map_states: dict[str, dict[str, ObjectState] | None] = {}
    for map_name in {o.map_name for o in map_objects if o.map_name}:
        if can_view_map is not None and not can_view_map(map_name):
            map_states[map_name] = None
            continue
    ...
    for obj in map_objects:
        ...
        entry = map_states.get(obj.map_name)
        if entry is None and obj.map_name in map_states:
            # Explicitly None → no permission for the referenced map
...
async def _get_map_states_batched(
    ...
    Objects are binned by kind (:func:`_bin_map_objects`) and each kind is
...
async def _get_object_state(connection: ConnectionBase, obj: MapObject) -> ObjectState:
    try:
        if obj.type == "host" and obj.host_name:
...
async def _get_presentation_states(
    ...
    Bound elements are adapted to transient ``MapObject``s and run through the
...
async def _get_folder_tree_states(cfg: MapConfig, connection: ConnectionBase) -> MapStates:
    """Resolve + aggregate a SETUP folder-tree map (concept v3).
    ...
    try:
        ...
        logger.exception("Folder-tree fetch failed for map %s", cfg.name)
        data = FolderTreeData()
        fetch_ok = False
...
# ---------------------------------------------------------------------------
# Flow Map topology snapshots — used by the WebSocket broadcast loop to
