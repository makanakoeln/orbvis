...
"""Parse legacy .cfg map content and convert it to an OrbVis map dict.
...
# map-level so objects only get a per-object override when set explicitly.
...
# NagVis geographic map sources that become an OrbVis worldmap (geo) map.
...
def _apply_global(map: dict[str, object], p: dict[str, str]) -> None:
    if "alias" in p:
        map["alias"] = p["alias"]
    if "map_image" in p:
        map["background_image"] = p["map_image"]
    # NagVis cfg uses the legacy "backend_id" parameter name on disk; map to our
    # canonical "connection_id" while reading.
    if "backend_id" in p:
        map["connection_id"] = p["backend_id"]
    if "iconset" in p:
        map["icon_size"] = ICONSET_SIZE.get(p["iconset"], 22)
...
def cfg_to_map(content: str, map_name: str) -> dict[str, object]:
    """Convert legacy .cfg text to an OrbVis map JSON dict."""
    map: dict[str, object] = {
        "name": map_name,
        "alias": map_name,
        "readonly": False,
