...
from cmk.orbvis_backend.schemas.map import AggregationNode
...
class ObjectState(BaseModel):
    ...
    # Populated only for type=='aggregation' when the MapObject has expand_depth > 0.
...
class FolderTreeNode(BaseModel):
    """A node in a SETUP folder-tree map (concept v3).
    ...
    # foldertree maps ship hosts only as tree nodes.
    services_summary: ServicesSummary | None = None
