...
"""FormSpec for the Global Settings admin surface (map / object defaults).

Two top-level groups feed the sidebar:

- ``map_defaults`` — selected when creating a new map.
...
from cmk.orbvis_backend.form_specs import OrbColorString, OrbDictGroup
from cmk.orbvis_backend.object_options import LINE_STYLES
...
_MAP_DEFAULTS = OrbDictGroup(
    title=Title("Map defaults"),
    help_text=Help("Pre-selected values when creating a new map."),
    key="map_defaults",
)
_OBJECT_APPEARANCE = OrbDictGroup(
    title=Title("Appearance"),
    help_text=Help("How objects are rendered by default on a new map."),
...
_OBJECT_TEMPLATES = OrbDictGroup(
    ...
    help_text=Help(
        "Global fallbacks used when a map or object defines no template "
...
def global_settings_spec(
    ...
    return Dictionary(
        ...
        help_text=Help(
            "Default values applied when creating new maps and objects. "
            "Per-map overrides take precedence."
        ),
        elements={
            # ── Map defaults ──
            ...
            "default_map_type": DictElement(
                ...
                parameter_form=SingleChoice(
                    ...
                    help_text=Help("Pre-selected map type in the 'New map' modal."),
                    elements=[
                        SingleChoiceElement(name="static", title=Title("Static map")),
                        SingleChoiceElement(name="worldmap", title=Title("Geo map")),
                        SingleChoiceElement(name="flow", title=Title("Flow map")),
            ...
            # pin it here once instead of pasting the URL into every Geo map.
            # ``None`` keeps each map's per-map ``tile_url`` choice intact.
            "default_tile_url": DictElement(
                group=_MAP_DEFAULTS,
                parameter_form=String(
                    title=Title("Default tile URL (Geo maps)"),
                    help_text=Help(
                        "Map-tile server applied to new Geo maps. Use the OSM-style "
                        "template syntax with {z}/{x}/{y}. Leave unset to let each "
                        "map pick its own default."
                    ),
            ...
            # Elements come from cmk.orbvis_backend.object_options.LINE_STYLES — the
            ...
            # ``label_*`` keys still used by every map renderer; the
