...
from cmk.orbvis_backend.core.middleware import (
...
from cmk.orbvis_backend.api.v1 import (
    auth,
    maps,
    connections,
    images,
...
from cmk.orbvis_backend.connections.test import TestConnection
from cmk.orbvis_backend.core.config import settings
from cmk.orbvis_backend.core.database import _connect, _ensure_schema
from cmk.orbvis_backend.core.security import hash_password
...
if FORM_SPECS_AVAILABLE:
    from cmk.orbvis_backend.form_specs._helpers import set_localizer
from cmk.orbvis_backend.schemas.connection import ConnectionConfig
from cmk.orbvis_backend.seed_maps import seed_demo_maps
from cmk.orbvis_backend.seed_images import seed_builtin_images
from cmk.orbvis_backend.services import map_service, connection_service, settings_service
from cmk.orbvis_backend.services.state_service import register_connection
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    map_service.migrate_legacy_keys()
    ...
    await asyncio.gather(
        asyncio.to_thread(seed_demo_maps, Path(settings.maps_dir)),
        asyncio.to_thread(seed_builtin_images, Path(settings.maps_dir).parent / "images"),
    )
...
app.include_router(map_tiles.router, prefix="/api/v1/maps", tags=["maps"])
...
_images_dir = Path(settings.maps_dir).parent / "images"
