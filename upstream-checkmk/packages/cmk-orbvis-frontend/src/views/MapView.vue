<template>
  <div class="orb-map">
    ...
    <div v-if="!isKiosk && !isPreview" class="orb-map__topbar">
      <!-- Left: Checkmk-style breadcrumb -->
      <div class="orb-map__crumb">
        <CmkBreadcrumb :items="breadcrumbItems" />
      </div>

      <div
        class="orb-map__topbar-actions"
        :class="drawerObject ? 'orb-map__topbar-actions--dimmed' : ''"
      >
        ...
        <span
          ...
          class="orb-map__pill"
          :class="
            flowProblems.critical > 0 ? 'orb-map__pill--critical' : 'orb-map__pill--warning'
          "
          :title="
            _t(
        ...
        <div
          class="orb-map__pill"
          :class="statesStore.connected ? 'orb-map__pill--ok' : 'orb-map__pill--offline'"
        >
          <span
            class="orb-map__dot"
            :class="statesStore.connected ? 'orb-map__dot--ok' : 'orb-map__dot--offline'"
          />
        ...
        <button
          ...
          class="orb-map__topbtn"
          :class="statesStore.notificationsEnabled ? 'orb-map__topbtn--alert' : ''"
          :title="
        ...
        <!-- Read-only badge -->
        <span v-if="mapConfig?.readonly" class="orb-map__badge orb-map__badge--readonly">
          <svg
            style="width: 10px; height: 10px"
            fill="none"
            viewBox="0 0 24 24"
        ...
        <!-- Editing badge -->
        <span v-if="editor.editMode.value" class="orb-map__badge orb-map__badge--editing">
          <svg
            style="width: 10px; height: 10px"
            fill="none"
            viewBox="0 0 24 24"
        ...
        <!-- Rotation countdown -->
        <button
          v-if="mapConfig && mapConfig.rotation_interval > 0 && rotationCountdown > 0"
          :title="rotationPaused ? _t('Resume rotation') : _t('Pause rotation')"
          class="orb-map__pill orb-map__pill--rotation"
          :class="rotationPaused ? 'orb-map__pill--paused' : 'orb-map__pill--ok'"
          @click="toggleRotationPause"
        >
          <svg
            style="width: 10px; height: 10px"
            :class="rotationPaused ? '' : 'orb-map__spin'"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2.5"
        ...
        <!-- Settings button (editors only, not for read-only maps) -->
        <button
          v-if="canEdit && !mapConfig?.readonly"
    ...
    <!-- Map area + optional edit panel. Acts as the portal target for the
             ...
             the topbar. -->
    <div id="orbvis-map-shell" class="orb-map__shell" data-tour="map-canvas">
      <!-- Loading overlay (covers all map types) -->
      <div v-if="isLoading" class="orb-map__loading">
        <CmkLoading />
      ...
      <div
        v-if="isWorldmap"
        class="orb-map__area orb-map__area--canvas"
        :class="{ 'worldmap-pick-active': worldmapPickActive }"
        @click="closeWorldmapMenus"
      >
        <div
          v-if="mapsStore.error"
          class="orb-map__msg orb-map__msg--overlay orb-map__msg--sm"
        >
          <span class="orb-map__error-text">{{ mapsStore.error }}</span>
          <router-link to="/" class="orb-map__backlink">
            {{ _t('← Back to overview') }}
          </router-link>
        </div>
        <WorldMapCanvas
        ...
        <MapSearch
          v-if="
            mapConfig &&
            mapConfig.objects.length > 0 &&
            !editor.editMode.value &&
            !mapsStore.error &&
            !isPreview
          "
          v-model="mapFilterNeedle"
        >
          <template #trailing>
        ...
        <!-- Fit all button -->
        <button
          v-if="mapConfig && !isPreview && mapConfig.objects.some((o) => o.lat != null)"
          :title="_t('Fit all objects')"
          class="leaflet-control-fit-all orb-map__fit-all"
        ...
        <div
          v-else-if="!mapConfig"
          class="orb-map__msg orb-map__msg--overlay orb-map__msg--muted"
        >
          <span>{{ _t('Map not found') }}</span>
          <router-link to="/" class="orb-map__backlink">
            {{ _t('← Back to overview') }}
          </router-link>
        </div>
        <div v-if="worldmapPickActive" class="orb-map__pick-banner" @click.stop>
          <span class="orb-map__pick-text">{{ _t('Pan/zoom the map, then apply.') }}</span>
      ...
      <!-- Radar -->
      <div v-else-if="isRadar" class="orb-map__area">
        <RadarCanvas
          :states="statesStore.states"
          :checkmk-url="checkmkUrl"
          :readonly="isKiosk || isPreview || (mapConfig?.readonly ?? false)"
          :compact="isPreview"
        ...
        />
        <MapSearch
          v-if="mapConfig && !editor.editMode.value && !isPreview"
          v-model="mapFilterNeedle"
        >
          <template #trailing>
            <ProblemsOnlyToggle v-model="problemsOnly" />
      ...
      <!-- Folder tree -->
      <div v-else-if="isFolderTree" class="orb-map__area">
        <FolderTreeMap
          :view="
            mapConfig && mapConfig.view.type === 'foldertree'
              ? mapConfig.view
              : { type: 'foldertree' }
          "
          :preview="isPreview"
          :kiosk="isKiosk"
          v-bind="mapConfig ? { mapName: mapConfig.name } : {}"
      ...
      <!-- Flowmap -->
      <div v-else-if="isFlowmap" class="orb-map__area">
        <FlowMap
          v-if="mapConfig?.connection_id"
          ref="flowMapRef"
          :connection-id="mapConfig.connection_id"
          :service-layout="serviceLayout"
          :readonly="isKiosk || isPreview || (mapConfig?.readonly ?? false)"
          :click-action="mapConfig.click_action"
          :checkmk-url="checkmkUrl"
          :flow-view="mapConfig.view.type === 'flow' ? mapConfig.view : null"
          :preview="isPreview"
          :problems-only="problemsOnly"
          :hover-template="mapConfig.hover_template ?? null"
          :context-template="mapConfig.context_template ?? null"
        ...
        />
        <div v-else class="orb-map__noconn">
          {{ _t('No connection configured for this map.') }}
        </div>
      </div>

      <!-- Presentation (design-first slide) -->
      <div v-else-if="isPresentation" class="orb-map__area orb-map__area--canvas">
        <PresentationCanvas
          v-if="mapConfig"
          :config="mapConfig"
          :states="statesStore.states"
          :edit-mode="editor.editMode.value"
          :readonly="mapConfig.readonly ?? false"
          :preview="isPreview"
          :kiosk="isKiosk"
          @object-hover="(obj, e) => objectHover.open(obj, e)"
          @object-hover-leave="objectHover.scheduleClose()"
      ...
      <div
        v-else
        class="orb-map__area orb-map__area--canvas orb-map__area--scroll"
        @click="onContainerClick"
      >
        <div
          v-if="mapsStore.error"
          class="orb-map__msg orb-map__msg--fill orb-map__msg--sm"
        >
          <span class="orb-map__error-text">{{ mapsStore.error }}</span>
          <router-link to="/" class="orb-map__backlink">
            {{ _t('← Back to overview') }}
        ...
        <template v-else-if="mapConfig">
          <MapSearch
            v-if="mapConfig.objects.length > 0 && !editor.editMode.value && !isPreview"
            v-model="mapFilterNeedle"
          >
            <template #trailing>
              <ProblemsOnlyToggle v-model="problemsOnly" />
          ...
          <!-- Empty map hint -->
          <div
            v-if="mapConfig.objects.length === 0 && !editor.editMode.value"
            class="orb-map__empty"
          >
            <svg
              class="orb-map__empty-icon"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="1.5"
            ...
            <p class="orb-map__empty-text">
              <template v-if="canEdit">{{
                _t('No objects yet — click the Edit button (bottom right) to add some')
              }}</template>
              <template v-else>{{ _t('This map has no objects yet') }}</template>
            </p>
          </div>
          <MapCanvas
            ref="canvasRef"
        ...
        <div v-else class="orb-map__msg orb-map__msg--fill orb-map__msg--muted">
          <span>{{ _t('Map not found') }}</span>
          <router-link to="/" class="orb-map__backlink">
            {{ _t('← Back to overview') }}
          </router-link>
      ...
      <DetailDrawer
        ...
        :readonly="mapConfig?.readonly ?? false"
        portal-target="#orbvis-map-shell"
        @close="closeDetail"
    ...
    <Teleport to="body">
      <div
        v-if="
          ...
          !drawerObject
        "
        class="orb-map__fab-stack"
      >
        <!-- Add Object panel — expands upward from FAB -->
        <Transition
          enter-from-class="orb-map__pop-enter-from"
          enter-active-class="orb-map__pop-enter-active"
          leave-to-class="orb-map__pop-leave-to"
          leave-active-class="orb-map__pop-leave-active"
          @after-leave="editor.resetDraft()"
        >
          <div
            v-if="editor.editMode.value && !!editor.draft.type && !isPresentation"
            class="orb-map__edit-panel"
        ...
        >
          <div
            ...
            class="orb-map__fab-wrap"
          >
            <Transition
              enter-from-class="orb-map__menu-enter-from"
              enter-active-class="orb-map__menu-enter-active"
              leave-to-class="orb-map__menu-leave-to"
              leave-active-class="orb-map__menu-leave-active"
            >
              <div
                v-if="addPickerOpen"
                class="orb-map__addmenu"
                role="menu"
              ...
              >
                <button
                  ...
                  :key="opt.name"
                  role="menuitem"
                  class="orb-map__addmenu-item"
                  @click="chooseAddType(opt.name)"
                >
                  <svg
                    class="orb-map__addmenu-icon"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2.5"
            ...
            <button
              ...
              class="orb-map__fab orb-map__fab--primary"
              :title="_t('Add Object')"
        ...
        <!-- FAB: Grid snap (edit mode, canvas maps only) — popover with sizes -->
        <div
        ...
        >
          ...
          <button
            class="orb-map__fab"
            :class="gridSnapActive ? 'orb-map__fab--active' : 'orb-map__fab--idle'"
            :title="_t('Grid')"
        ...
        <button
          data-tour="edit-fab"
          class="orb-map__fab"
          :class="editor.editMode.value ? 'orb-map__fab--active' : 'orb-map__fab--idle'"
    ...
    <Teleport to="body">
      <Transition
        enter-from-class="orb-map__bar-enter-from"
        enter-active-class="orb-map__bar-enter-active"
        leave-to-class="orb-map__bar-leave-to"
        leave-active-class="orb-map__bar-leave-active"
      >
        <div
          v-if="
            editor.editMode.value &&
            ...
            actionBarStyle
          "
          class="orb-map__actionbar"
          :style="actionBarStyle"
        >
          <span class="orb-map__actionbar-type">{{
            editor.selectedCount.value > 1
              ? _t('%{n} selected', { n: editor.selectedCount.value })
              : selectedObject!.type
          }}</span>
          <div class="orb-map__actionbar-divider" />
          <button
            v-if="editor.selectedCount.value <= 1"
            title="Edit properties"
            class="orb-map__actionbar-btn orb-map__actionbar-btn--edit"
          ...
          <!-- Detach: quick action for a line bound to an object (geo + static). -->
          <button
            v-if="
              editor.selectedCount.value <= 1 &&
              selectedObject?.type === 'line' &&
              (selectedObject.start_ref || selectedObject.end_ref)
            "
            :title="_t('Detach from object')"
            class="orb-map__actionbar-btn"
            @click="onObjectDetach(selectedObject!)"
          >
            <svg
              style="width: 14px; height: 14px"
              fill="none"
          ...
          <button
            ...
            class="orb-map__actionbar-btn orb-map__actionbar-btn--danger"
    ...
    <!-- Services-layout toggle (Flow Map only). Hidden during triage. -->
    <Teleport to="body">
      <div v-if="isFlowmap && !drawerObject" class="orb-map__svc">
        <div class="orb-map__svc-wrap">
          <!-- Backdrop to close dropdown on outside click -->
          <div
            v-if="serviceLayoutOpen"
            class="orb-map__svc-backdrop"
            @click="serviceLayoutOpen = false"
          />

          <button
            class="orb-map__svc-btn"
            :class="serviceLayout !== 'off' ? 'orb-map__svc-btn--on' : 'orb-map__svc-btn--off'"
            @click="serviceLayoutOpen = !serviceLayoutOpen"
          >
            {{ _t('Services') }}
            <svg
              style="width: 10px; height: 10px"
              class="orb-map__svc-caret"
              :class="serviceLayoutOpen ? 'orb-map__svc-caret--open' : ''"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="2.5"
          ...
          <Transition
            ...
            leave-active-class="orb-map__menu-leave-active"
          >
            <div v-if="serviceLayoutOpen" class="orb-map__svc-menu">
              <button
                v-for="opt in serviceLayoutOptions"
                :key="opt.value"
                class="orb-map__svc-item"
                :class="serviceLayout === opt.value ? 'orb-map__svc-item--active' : ''"
    ...
    <HoverMenu
      ...
      :connection-id="
        isFolderTree
          ? null
          : (objectHover.hover.object.connection_id ?? mapConfig?.connection_id ?? null)
      "
      :checkmk-url="checkmkUrl"
      :template="
        isFolderTree
    ...
    <template v-if="isFolderTree && folderCtx">
      ...
      <div
        ...
        @click.stop
      >
        <div class="orb-map__ctx-header">
          <p class="orb-map__ctx-name">
            {{ folderCtx.node.title || 'Main' }}
          </p>
          <p class="orb-map__ctx-type">
            {{ _t('Folder') }}
          </p>
        </div>
        <button
          ...
          type="button"
          class="orb-map__ctx-item"
          @click="onFolderCtxBulk"
        >
          <svg
            class="orb-map__ctx-icon"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2"
    ...
    <div
      ...
      @click.stop
    >
      <button type="button" class="orb-map__ctx-item" @click="onWorldmapCanvasCtxSaveAsDefault">
        <svg
          class="orb-map__ctx-icon"
          fill="none"
          viewBox="0 0 24 24"
    ...
    <Teleport to="body">
      <ObjectPropertiesModal
        ...
        :map-icon-size="mapConfig?.icon_size ?? settingsStore.settings.icon_size"
    ...
    <OnboardingTour
      v-if="showMapTour && auth.user && capabilities.tour"
      :steps="mapTourSteps"
      :storage-key="`orbvis_map_toured_${auth.user.user_id}`"
      @close="showMapTour = false"
      @step-click="onMapTourStepClick"
      @step-back="onMapTourStepBack"
    />
  </div>
</template>
...
import AckModal from '@/components/map/AckModal.vue'
import MapCanvas from '@/components/map/MapCanvas.vue'
import MapSearch from '@/components/map/MapSearch.vue'
import MapSettingsModal from '@/components/map/MapSettingsModal.vue'
import BulkAckModal from '@/components/map/BulkAckModal.vue'
import BundleDialog from '@/components/map/BundleDialog.vue'
import CommentModal from '@/components/map/CommentModal.vue'
import ContextMenu from '@/components/map/ContextMenu.vue'
import DetailDrawer from '@/components/map/DetailDrawer.vue'
import DowntimeModal from '@/components/map/DowntimeModal.vue'
import EditPanel from '@/components/map/EditPanel.vue'
import FlowMap from '@/components/map/FlowMap.vue'
import FolderBulkActionModal from '@/components/map/FolderBulkActionModal.vue'
import FolderTreeMap from '@/components/map/FolderTreeMap.vue'
...
import ProblemsOnlyToggle from '@/components/map/ProblemsOnlyToggle.vue'
import RadarCanvas from '@/components/map/RadarCanvas.vue'
import RemoveDowntimeModal from '@/components/map/RemoveDowntimeModal.vue'
import WorldMapCanvas from '@/components/map/WorldMapCanvas.vue'
import PresentationCanvas from '@/components/map/presentation/PresentationCanvas.vue'
...
import { mapsApi, connectionsApi } from '@/api/client'
import { useMapEditor } from '@/composables/useMapEditor'
import { useMapFabMenus } from '@/composables/useMapFabMenus'
import { useMapFullscreen } from '@/composables/useMapFullscreen'
import { useMapRotation } from '@/composables/useMapRotation'
import { useMapTour } from '@/composables/useMapTour'
...
import type { MapObject, BulkAckTarget, ObjectState, ServiceLayout } from '@/types/api'
import { buildCheckmkUrl, openUrl } from '@/utils/mapNavigation'
import { getMapObjectIdentifier, getMapObjectName, getObjectTypeLabel } from '@/utils/naming'
...
import useClickOutside from '@cmk/lib/useClickOutside'
...
const { openKioskInNewTab, enterFullscreen, exitFullscreen } = useMapFullscreen(
  mapName,
  isKiosk
)

const mapConfig = computed(() => mapsStore.currentMap)
// Whether the current user may edit *this* map: admins always, otherwise the
// per-map capability the backend stamps on the map-list entry.
const canEdit = computed(
  () =>
    auth.isAdmin || mapsStore.maps.find((b) => b.name === mapName.value)?.can_edit === true
)
const mapConfigAsRead = computed<import('@/types/api').MapRead | null>(() => {
  const cfg = mapsStore.currentMap
  if (!cfg) return null
...
// Top-right search bar shared by static / worldmap / radar maps.
// FlowMap ships its own search because it filters d3 nodes directly.
const mapFilterNeedle = ref('')
watch(mapName, () => {
  mapFilterNeedle.value = ''
})
const isLoading = computed(
  () => mapsStore.loading || (statesStore.initialLoad && !mapsStore.error)
)
...
const { showMapTour, mapTourSteps, onMapTourStepClick, onMapTourStepBack } =
  useMapTour(editor)
...
const deleteDialogTitle = computed(() => {
  ...
  const name = obj.label?.text || getMapObjectIdentifier(obj)
...
function onObjectDblclick(obj: MapObject) {
...
function onObjectDuplicate(obj: MapObject) {
  editor.selectObject(obj.id)
  editor.duplicateSelected()
}

function onObjectStraighten(obj: MapObject) {
  void editor.updateObjectProperties(obj.id, { mid_x: null, mid_y: null })
}

function onObjectDetach(obj: MapObject) {
...
// Writing the picked map view into map settings is the settings cluster's
...
const {
  ...
  editor,
  isReadonly: () => mapConfig.value?.readonly ?? false,
  openPropsModal,
...
watch(mapName, () => closeWorldmapMenus())
...
// ---- Detail drawer (shared across static / worldmap / radar maps) ----

const detailDrawerObject = ref<MapObject | null>(null)
...
function openDetail(obj: MapObject) {
...
// All host MapObjects on this map, keyed by hostname so the Drawer's
...
// Presentation view mode: bound elements surface as transient MapObjects —
// click follows the shared object-click contract (click_action, Ctrl+Click,
// detail drawer), same as the other map types.
function onPresentationObjectClick(obj: MapObject, event?: MouseEvent): void {
  objectHover.close()
...
// static map's openContextMenu).
function onPresentationContext(obj: MapObject, event: MouseEvent): void {
...
function onWorldmapMarkerContext(obj: MapObject, x: number, y: number): void {
  editor.selectObject(obj.id)
...
const {
  ...
  checkmkUrl,
  canBulkCommand,
  isClickActionNone: () => mapConfig.value?.click_action === 'none',
  onObjectClick,
  setDrawerSeed: (state) => {
...
// next reload reuses the editor's divisor (see MapConfig.canvas_width). Only
...
function onObjectClick(obj: MapObject, event?: MouseEvent) {
  ...
  if (mapConfig.value?.click_action === 'none') return
  if (obj.url) {
    openUrl(obj.url, obj.url_target || '_blank')
    return
  }
  if (obj.type === 'map' && obj.map_name) {
    void router.push({ name: 'map', params: { name: obj.map_name } })
    return
  }
...
function objectHasMonitoringTarget(obj: MapObject): boolean {
  switch (obj.type) {
    case 'host':
    case 'service':
    case 'line':
...
async function onGraphResizeEnd(id: string, width: number, height: number) {
  const obj = mapConfig.value?.objects.find((o) => o.id === id)
  if (!obj) return
  if (obj.type === 'textbox') {
...
async function onStartPlacing() {
  ...
  const connectionId = mapConfig.value?.connection_id
  if (mapConfig.value?.view.type === 'worldmap' && connectionId && d.host_name) {
    try {
...
const flowDrawerObject = ref<MapObject | null>(null)
const flowMapRef = ref<{ closeDetail: () => void } | null>(null)
const drawerObject = computed<MapObject | null>(
  () => detailDrawerObject.value ?? flowDrawerObject.value
)
function closeAnyDrawer(): void {
  if (detailDrawerObject.value) closeDetail()
  if (flowDrawerObject.value) flowMapRef.value?.closeDetail()
}

const breadcrumbItems = computed<BreadcrumbItem[]>(() => {
  const mapTitle = mapConfig.value?.alias || String(route.params.name ?? '')
  const items: BreadcrumbItem[] = [{ title: _t('Maps'), to: '/' }]
  if (drawerObject.value) {
    items.push({ title: mapTitle, onClick: closeAnyDrawer })
    items.push({ title: getMapObjectName(drawerObject.value) })
  } else {
    items.push({ title: mapTitle })
  }
  return items
})
...
async function persistView(patch: Record<string, unknown>): Promise<void> {
  ...
  cfg.view = newView
  try {
    const updated = await mapsApi.update(cfg.name, { view: newView }, token)
    ...
    console.warn('Failed to persist map view:', err)
  }
}
...
const problemsOnly = computed<boolean>({
  get: () => {
    const view = mapConfig.value?.view as { problems_only?: boolean } | undefined
...
// map config swaps. The watch on mapName below clears the ref before the
// new map arrives, so this fires once the new mapConfig is in place.
watch(
  () => mapConfig.value?.view,
  (view) => {
...
function openSettings() {
  ...
  settingsParentMapSize.value =
    mapConfig.value.view.type === 'worldmap'
...
async function onSettingsUpdated() {
  ...
  stopRotation()
  scheduleRotation(mapsStore.currentMap?.rotation_interval ?? 0)
...
watchEffect(async () => {
  ...
  if (
    ...
    cfg &&
    !cfg.readonly &&
    !localStorage.getItem(`orbvis_map_toured_${auth.user.user_id}`)
  ) {
    showMapTour.value = true
  }
})
...
function onPreviewMessage(ev: MessageEvent) {
  ...
  const cur = mapsStore.currentMap
  if (!cur) return
...
onMounted(() => {
  if (auth.canConfigure || auth.canCreateMaps) connectionsStore.fetchConnections()
  // The map list carries per-map can_edit; ensure it's loaded even on a
  // direct deep-link so non-admin editors get their edit affordances.
  if (mapsStore.maps.length === 0) void mapsStore.fetchMaps()
  document.addEventListener('keydown', onKeyDown)
  window.addEventListener('message', onPreviewMessage)
})

watch(
  () => mapsStore.currentMap?.name,
  (name) => {
...
.orb-map__topbar-actions--dimmed {
  opacity: 0.4;
}

.orb-map__topbar-actions--dimmed:hover {
  opacity: 1;
}
...
.orb-map__dot--ok {
  ...
  animation: orb-map-pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite;
}

.orb-map__dot--offline {
  background: var(--color-light-red-40);
}

@keyframes orb-map-pulse {
  50% {
    opacity: 0.5;
  }
}
...
.orb-map__spin {
  animation: orb-map-spin 3s linear infinite;
}

@keyframes orb-map-spin {
  to {
...
.orb-map__topbtn:hover {
  color: var(--text);
  background: var(--bg-hover);
}

.orb-map__topbtn--alert,
.orb-map__topbtn--alert:hover {
  color: var(--color-yellow-50);
}
...
.orb-map__badge--readonly {
  color: var(--text-muted);
  background: var(--bg-hover);
  box-shadow: 0 0 0 1px var(--default-border-color);
}

.orb-map__badge--editing {
  color: var(--color-yellow-50);
  background: color-mix(in srgb, var(--color-warning) 10%, transparent);
...
.orb-map__area--canvas {
  background: var(--bg);
}

.orb-map__area--scroll {
  overflow: auto;
}
...
.orb-map__fit-all:hover {
  background: var(--color-mist-grey-0);
}

.orb-map__pick-banner {
  position: absolute;
  top: var(--dimension-6);
  left: 50%;
  z-index: 2000;
...
.orb-map__pop-enter-from,
.orb-map__pop-leave-to {
  opacity: 0;
  transform: translateY(16px) scaleX(0.95) scaleY(0.75);
}

.orb-map__pop-enter-active {
  transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1);
  transform-origin: bottom right;
}

.orb-map__pop-leave-active {
  transition: all 0.2s cubic-bezier(0.4, 0, 1, 1);
  transform-origin: bottom right;
}

.orb-map__menu-enter-from,
.orb-map__menu-leave-to {
  opacity: 0;
  transform: translateY(4px) scale(0.95);
}

.orb-map__menu-enter-active {
  transition: all 0.15s cubic-bezier(0, 0, 0.2, 1);
  transform-origin: bottom right;
}

.orb-map__menu-leave-active {
  transition: all 0.1s cubic-bezier(0.4, 0, 1, 1);
  transform-origin: bottom right;
}

.orb-map__bar-enter-from,
.orb-map__bar-leave-to {
  opacity: 0;
  transform: translateY(4px) scale(0.95);
}

.orb-map__bar-enter-active {
  transition: all 0.15s cubic-bezier(0, 0, 0.2, 1);
}

.orb-map__bar-leave-active {
  transition: all 0.1s cubic-bezier(0.4, 0, 1, 1);
}
...
.orb-map__addmenu-item:hover {
  background: var(--bg-hover);
}

.orb-map__addmenu-icon {
  flex-shrink: 0;
  width: 12px;
  height: 12px;
  color: var(--text-muted);
...
.orb-map__svc-item:hover {
  color: var(--text);
  background: var(--bg-hover);
}

.orb-map__svc-item--active,
.orb-map__svc-item--active:hover {
  color: var(--color-corporate-green-40);
  background: color-mix(in srgb, var(--color-corporate-green-50) 10%, transparent);
}

.orb-map__ctx-backdrop {
  position: fixed;
  inset: 0;
  z-index: 40;
}
