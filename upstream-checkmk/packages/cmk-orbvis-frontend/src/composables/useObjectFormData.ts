...
import type { MapObject, MetricChoice, MetricGraphGroup, ObjectState } from '@/types/api'
 ...
 * IDs (and their human titles), graph templates, map names, host/service/
...
export function useObjectFormData(options: ObjectFormDataOptions) {
  ...
  async function fetchMapNames() {
    ...
    const maps = await mapsApi.list(auth.accessToken).catch(logLoadError('map names'))
    mapNames.value = maps.map((b) => b.name)
    mapLabels.value = maps.map((b) => b.alias || b.name)
  }

  // ---- Graph source mode ----

  function deriveGraphSource(obj: MapObject): GraphSource {
  ...
  // per-object connection — otherwise the dropdown stays bound to the map's
  ...
  async function loadAutocomplete() {
    // Per-object connection override wins; fall back to the map default.
  ...
  onMounted(() => {
    ...
    if (object().type === 'map') fetchMapNames()
  })
