 ...
 */
export function usePresentationDocument(config: () => MapConfig, onMapSwitch?: () => void) {
  const { _t } = usei18n()
  ...
  async function saveNow(): Promise<void> {
    ...
    try {
      ...
      const cfg = await mapsApi.update(name, payload, auth.accessToken, version.value)
      version.value = cfg.version ?? version.value
      if (mapsStore.currentMap?.name === cfg.name) {
        mapsStore.currentMap.version = cfg.version ?? 0
        mapsStore.currentMap.view = cfg.view
      }
      savedAt.value = true
      ...
      // without tearing the working copy down (fetchMap nulls currentMap,
      // which would unmount the editor and drop in-progress edits).
      try {
        const cfg = await mapsApi.get(name, auth.accessToken)
