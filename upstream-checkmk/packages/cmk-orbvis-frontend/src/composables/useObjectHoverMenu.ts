...
interface OpenOptions {
   ...
   * `null` disables (lines span the whole map); omitted = derived from
   * the event target. */
...
interface HoverMenuOptions {
  ...
  resolveState?: (obj: MapObject) => ObjectState | undefined
...
export function useObjectHoverMenu(options: HoverMenuOptions = {}) {
  ...
  function open(obj: MapObject, event: MouseEvent | null, opts: OpenOptions = {}): void {
    grace.cancelClose()
    hover.object = obj
