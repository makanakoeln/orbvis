...
import type { MapConfig, MapObject } from '@/types/api'
...
function makeObject(o: Partial<MapObject> & { id: string; type: string }): MapObject {
...
describe('useMapEditor line drag — bound endpoints', () => {
  ...
  beforeEach(() => {
    ...
    mapName = ref('map1')
    mockMapsApi.updateObject.mockClear()
  })
  ...
  it('seeds a move drag of a bound line from the live object, not the stale stored coord', () => {
    ...
    store.currentMap = mapWithStaleBoundLine()
    const editor = useMapEditor(mapName, async () => {})
    const line = store.currentMap.objects.find((o) => o.id === 'line1')!
