...
import type { MapListView } from '@/types/api'
...
export function useMapListViewMode() {
  ...
  function viewModeStorageKey(): string {
    return `orbvis_map_list_view_${auth.user?.user_id ?? 'anon'}`
  }
  ...
  const viewMode = computed<MapListView>(
    () =>
      localViewMode.value ??
      (settingsStore.settings.map_list_view === 'table' ? 'table' : 'cards')
  )
  ...
  function setViewMode(value: string) {
    const next: MapListView = value === 'table' ? 'table' : 'cards'
