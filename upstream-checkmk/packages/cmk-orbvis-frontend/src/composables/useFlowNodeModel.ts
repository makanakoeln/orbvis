...
import type { MapObject, ObjectState, ServiceNode, TopologyNode } from '@/types/api'
...
export interface FNode extends SimulationNodeDatum {
  ...
  // same status detail (alias, services_summary, …) as the static map.
...
/**
 * Maps flow-map d3 nodes (FNode) to OrbVis MapObject/ObjectState (for the
...
export function useFlowNodeModel(options: FlowNodeModelOptions) {
  ...
  function mapObjectFromFNode(d: FNode): MapObject {
    if (d.nodeType === 'site') {
      return {
        id: d.id,
        type: 'site',
