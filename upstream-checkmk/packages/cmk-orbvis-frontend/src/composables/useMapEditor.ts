...
export function useMapEditor(mapName: Ref<string>, onMapChange: () => Promise<void>) {
  ...
  async function moveSelectedLayer(direction: 'front' | 'back') {
    const map = mapsStore.currentMap
    if (!map) return
    ...
    const zOf = (o: MapObject) => o.z ?? map.default_z ?? 1
    ...
    for (const obj of ordered) {
      ...
      obj.z = newZ
      try {
        await mapsApi.updateObject(mapName.value, obj.id, { z: newZ }, auth.accessToken!)
      } catch (e) {
  ...
  async function deleteAllSelected() {
    ...
    try {
      ...
      if (mapsStore.currentMap) {
        mapsStore.currentMap.objects = mapsStore.currentMap.objects.filter(
          (o) => !ids.includes(o.id)
        )
  ...
  function _clearDanglingRefs(removedIds: string[]) {
    for (const o of mapsStore.currentMap?.objects ?? []) {
  ...
  async function saveObjectPositions(moves: { id: string; x: number; y: number }[]) {
    ...
    try {
      for (const m of moves) {
        await mapsApi.updateObject(mapName.value, m.id, { x: m.x, y: m.y }, auth.accessToken!)
  ...
  async function ensureCanvasSize(width: number, height: number) {
    ...
    if (!w || !h || (map.canvas_width === w && map.canvas_height === h)) return
    try {
      ...
      map.canvas_width = w
      map.canvas_height = h
      if (typeof updated.version === 'number') map.version = updated.version
    } catch (e) {
      _reportError(_t('Saving the map size failed'), e)
    }
  }

  function _avoidOverlap(id: string, x: number, y: number, objects: readonly MapObject[]) {
  ...
  function _objectNearPoint(x: number, y: number, lineId: string): MapObject | null {
    const objects = mapsStore.currentMap?.objects ?? []
  ...
  async function endLineDrag() {
    ...
    try {
      await mapsApi.updateObject(mapName.value, t.id, updates, auth.accessToken!)
  ...
  // The shared part of a Draft → MapObject mapping: id, monitoring binding,
  ...
  function _draftToObject(id: string): MapObject {
  ...
  async function placeAt(x: number, y: number) {
    ...
    const existing = mapsStore.currentMap?.objects ?? []
    const placePos =
      draft.type === 'line'
  ...
  async function _persistNewObject(obj: MapObject) {
    try {
  ...
  async function moveObjectToLatLng(id: string, lat: number, lng: number, endpoint: 1 | 2 = 1) {
    ...
    try {
      await mapsApi.updateObject(mapName.value, id, keys, auth.accessToken!)
  ...
  async function bundleSelected(name: string, kind: 'static' | 'location') {
    ...
    try {
      await mapsApi.addObject(mapName.value, bundle, auth.accessToken!)
