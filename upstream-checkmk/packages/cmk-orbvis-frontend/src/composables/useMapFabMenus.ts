...
/**
 * The two edit-mode FAB popup menus on a map:
 *  - the "add object" type picker (anchored above the "+" FAB)
 *  - the grid-snap size menu
 *
 * Both are pure UI-state holders driven by the shared map editor. The
...
export function useMapFabMenus(editor: MapEditor) {
  ...
  // Leaving edit mode dismisses the picker so it can't linger over a read-only map.
  watch(
