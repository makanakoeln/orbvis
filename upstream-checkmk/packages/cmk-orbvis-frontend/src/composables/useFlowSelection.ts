...
interface FlowSelectionOptions {
  ...
  mapObjectFromFNode: (d: FNode) => MapObject
}

/**
 * Multi-select for the flow map: shift-click toggles a node, shift-drag
...
export function useFlowSelection(options: FlowSelectionOptions) {
  const { svgEl, hostHalo, mapObjectFromFNode } = options
  ...
  const selectedObjects = computed(() =>
    [...selectedFNodes.values()].map((d) => mapObjectFromFNode(d))
  )

  async function bulkAction(
    handler: (obj: MapObject | null) => Promise<void> | void
  ): Promise<void> {
