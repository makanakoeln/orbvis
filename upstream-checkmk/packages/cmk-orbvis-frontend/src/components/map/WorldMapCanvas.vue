...
// from the local on-disk cache and render near-instantly. Maps may override
...
function markerZBase(obj: MapObjectType): number {
...
function makeTextboxIcon(obj: MapObjectType, selected: boolean): L.DivIcon {
...
function makeDivIcon(obj: MapObjectType): L.DivIcon {
  ...
  const escapedName = escapeHtml(getMapObjectName(obj))
  ...
  // Dashed green outline like the static map, but markers are themselves
...
function resolveLineColor(obj: MapObjectType): string {
...
// maps there is no icon-edge inset, since a marker sits at a single lat/lng.
...
// long-open map doesn't hit 401 tiles.
