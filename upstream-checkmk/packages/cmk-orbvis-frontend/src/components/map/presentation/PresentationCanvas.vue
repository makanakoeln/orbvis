...
import { mapObjectFromElement } from '@/utils/presentationMapObject'
...
const emit = defineEmits<{
  'object-hover': [obj: MapObject, event: MouseEvent]
  'object-hover-leave': []
  'object-click': [obj: MapObject, event: MouseEvent | undefined]
  'object-context': [obj: MapObject, event: MouseEvent]
}>()
...
// View-mode drill-down: outside the editor, bound elements act like map
...
function viewObjectFor(el: PresentationElement): MapObject | null {
  if (!viewLinked(el) || !isBindable(el)) return null
  const obj = mapObjectFromElement(el)
...
// Effective connection of a bindable element: its own override or the map default.
...
// ── Template gallery ────────────────────────────────────────────────────────
// Greets an empty slide once per map and session; reopenable from the slide
...
// Slide-level changes (theme/size/background) are map metadata, not element
