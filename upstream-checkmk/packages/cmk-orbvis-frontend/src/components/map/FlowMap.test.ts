...
// The flow map pulls its data from the central WS pipeline via
...
vi.mock('@/api/client', () => ({
  ...
  mapsApi: { update: vi.fn(), states: vi.fn() },
...
// test focused on FlowMap's own d3 render pipeline.
const stubs = {
  MapSearch: true,
...
function mountWithTopology() {
  ...
  return mount(FlowMap, { props: baseProps, global: { plugins: [pinia], stubs } })
}
