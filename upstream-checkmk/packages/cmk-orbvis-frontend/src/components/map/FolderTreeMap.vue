...
import FolderTreeTreemap from '@/components/map/FolderTreeTreemap.vue'
import FolderTreeRow from '@/components/map/FolderTreeRow.vue'
...
const mapRef = useTemplateRef<InstanceType<typeof FolderTreeTreemap>>('mapRef')

// Persist a view-preference patch (view mode / "Problems only") so the map
// reopens the same way (mirrors the Flow map's services-layout control),
// best-effort and never in preview. The server's response — new version and
// merged view — is folded back into the shared map: otherwise each such write
// silently bumps the map version on disk while the client keeps the stale one,
...
function persistView(patch: Partial<FolderTreeView>) {
  if (props.preview || !props.mapName || !auth.accessToken) return
  void mapsApi
    .update(props.mapName, { view: { ...props.view, ...patch } }, auth.accessToken)
    .then((updated) => {
      const cur = maps.currentMap
...
async function fetchServices(name: string, hostPath: string, siteId: string | null) {
  if (!props.mapName || !auth.accessToken) return
  const svcs = await mapsApi.folderHostServices(props.mapName, name, auth.accessToken)
...
async function ensureServices(host: FolderTreeNode) {
  ...
  if (!showServices.value || !props.mapName || !auth.accessToken) return
...
async function runServiceSearch() {
  if (props.preview || !props.mapName || !auth.accessToken || !hasServiceSearch.value) {
    clearSearchServices()
  ...
  try {
    const res = await mapsApi.folderSearch(
      props.mapName,
...
// the operator has drilled into whenever the map's state refreshes, so leaf
...
/* Reserved, non-overlapping toolbar — wraps to a second line on narrow maps
