<template>
  <div
    ref="rootRef"
    class="map-search"
    :class="{ 'map-search--open': dropdownOpen, 'map-search--inline': inline }"
  >
    <svg
      fill="none"
      viewBox="0 0 24 24"
    ...
    <button
      type="button"
      class="map-search__icon-btn map-search__help"
      :class="{ 'map-search__help--active': dropdownOpen }"
    ...
    <div v-if="dropdownOpen" class="map-search__dropdown" role="listbox" @mousedown.prevent>
      <div class="map-search__dropdown-title">
        {{ _t("Type '/' to use a search operator") }}
      </div>
      <ul class="map-search__operator-list">
        <li
          v-for="op in operators"
          :key="op.prefix"
          role="option"
          class="map-search__operator"
          @click="applyOperator(op.prefix)"
        >
          <span class="map-search__operator-tag">{{ op.prefix }}:</span>
          <span class="map-search__operator-label">{{ op.label }}</span>
        </li>
      </ul>
      <div class="map-search__info">
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          class="map-search__info-icon"
        >
...
.map-search__operator:hover {
  background: rgb(255 255 255 / 6%);
}

.map-search__operator-tag {
  display: inline-flex;
  align-items: center;
