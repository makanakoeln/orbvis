...
defineEmits<{
  ...
  'subtree-enter': [obj: MapObject, state: ObjectState, event: MouseEvent]
  'subtree-leave': []
}>()
...
// Render-sink guard: map JSON predating the server-side scheme allowlist
...
const customIconStyle = computed(() => {
  ...
  // + soft glow + faint tint, so the status reads clearly on any map background
