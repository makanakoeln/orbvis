...
import FolderTreeMap from './FolderTreeMap.vue'

// The folder tree arrives via the central WS pipeline (statesStore.folderTree);
// mapsApi is only touched for view-preference persistence and lazy service
// fetches, both gated on a mapName prop we don't pass. Mock the API client so
...
function mountList() {
  ...
  return mount(FolderTreeMap, {
    props: { view: listView },
    global: { plugins: [pinia], stubs: { MapSearch: true } }
  })
}

describe('FolderTreeMap (list mode)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.stubGlobal(
      'ResizeObserver',
  ...
  it('renders the waiting placeholder when no folder tree arrived yet', () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const wrapper = mount(FolderTreeMap, {
      props: { view: listView },
      global: { plugins: [pinia], stubs: { MapSearch: true } }
    })
