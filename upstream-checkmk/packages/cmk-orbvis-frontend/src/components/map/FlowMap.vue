...
import MapZoomResetPill from '@/components/map/MapZoomResetPill.vue'
...
const emit = defineEmits<{
  ...
  (e: 'drawer-object', value: MapObject | null): void
...
const {
  ...
  bulkAction
} = useFlowSelection({ svgEl, hostHalo, mapObjectFromFNode })

const detailObject = ref<MapObject | null>(null)
...
function openDetail(obj: MapObject, fNode: FNode): void {
  cancelPendingFit()
...
// Hostnames the Flow Map currently renders — Drawer-Topology entries that
...
function onSelectFlowHost(hostName: string): void {
  ...
  if (!node) return
  // Synthesize a MapObject the Drawer can render — Flow Map nodes don't
  // come from mapConfig.objects, they live in the live topology snapshot.
  detailObject.value = {
...
// them in a click-to-fix CTA so a green-dot map doesn't mask CRIT/WARN.
...
// Lifted to MapView so the parent can hide overlapping bottom-right controls
...
// succession or multi-select drags would otherwise hammer maps.update.
...
// At fit-to-view on multi-hundred-host maps the initial scale lands around
...
function flushZoomTransform(): void {
  ...
  // source of stutter on dense maps.
}

// Reset auto-fit when switching maps
watch(
  () => props.connectionId,
  () => {
    _hasFitOnce = false
...
function render(svg: SVGSVGElement, topoNodes: TopologyNode[]) {
  ...
  // the saved map view, so reload preserves the operator's layout.
  ...
  siteIds.forEach((sid, i) => {
    ...
    // small maps and clears the dispersion-induced spread on large
  ...
  // starting point. On flat maps we group hosts per site and lay each
  // group out in a half-disk below its site root; on hierarchical maps
  ...
  simulation = forceSimulation<FNode>(fNodes)
    ...
    // Faster alpha decay on flat maps: the severity-spiral pre-layout
  ...
  const dragBehavior = drag<SVGGElement, FNode>()
    ...
    .on('end', (event, d) => {
      ...
      // Readonly maps (demos) allow interactive drags but don't
  ...
  const linkMerge = linkEnter
    ...
    .attr('stroke', (d) => {
      ...
      // light and dark map backgrounds rather than a fixed faint grey.
  ...
  const nodeEnter = nodeSel
    ...
    .attr('cursor', (d) => {
      // Readonly maps (e.g. the bundled demos) still allow drags so
    ...
    .on('click', (event: MouseEvent, d) => {
      ...
      if (d.nodeType === 'site') {
        objectHover.close()
        openDetail(mapObjectFromFNode(d), d)
        return
      }
    ...
    .on('contextmenu', (event: MouseEvent, d) => {
      ...
      objectHover.close()
      contextMenu.object = mapObjectFromFNode(d)
    ...
    .on('mouseenter', (event: MouseEvent, d) => {
      ...
      hoverFNode.value = d
      objectHover.open(mapObjectFromFNode(d), null, {
        x: nodeRect.right + 8,
        y: nodeRect.top,
        anchorRect: {
          left: nodeRect.left,
  ...
  // Readonly maps keep drag interactive (persistence inside drag.end
  ...
  // maps even though nothing structurally changed.
  ...
  if (isInitial || structurallyChanged) {
    // Lower initial alpha on flat maps: pre-layout already approximates
    // the steady state, so 0.4 settles in ~25 ticks instead of ~75 from
    // alpha=1. Hierarchical maps still need full-energy initial sim.
