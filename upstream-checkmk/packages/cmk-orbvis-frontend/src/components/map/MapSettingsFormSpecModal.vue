  ...
  >
    <div class="map-settings__shell">
      <div class="map-settings__layout">
        <div class="map-settings__body">
          ...
          <div class="map-settings__scroll">
            <!-- General -->
            <div v-if="activeTab === 'general'" ref="generalFormEl" class="map-settings__form">
              <!-- Generic metadata (Identification, Display, Behavior,
                         Templates) renders first so the operator can name and
                         wire up the map before tuning type-specific
              ...
              <div v-if="templatePreview" class="map-settings__template-preview">
                <span class="map-settings__template-preview-label">{{
                  _t('Hover preview:')
                }}</span>
              ...
              <div
                ...
                class="map-settings__type-section map-settings__stack"
              >
              ...
              <ul v-if="errorMessages.length" class="map-settings__errors">
            ...
            <div v-else-if="activeTab === 'permissions'">
              ...
              <div v-if="isCmkDeployment" class="map-settings__perm-cmk">
                <p class="map-settings__perm-cmk-intro">
                  {{
                    _t(
                      'Map view and edit are gated by Checkmk role permissions (orbvis.see, orbvis.edit). Manage them in Checkmk WATO.'
                    )
                  }}
                </p>
              ...
              <div v-else-if="permLoading" class="map-settings__perm-loading">
                <CmkLoading />
              </div>
              <div v-else>
        ...
        <aside v-if="showPreview" class="map-settings__preview">
          ...
          <div class="map-settings__preview-stage">
            ...
            />
            <div v-if="previewLoading" class="map-settings__preview-loading">
              <CmkLoading />
            </div>
          </div>
        </aside>
      ...
      <div class="map-settings__footer">
        ...
        <CmkButton v-if="auth.canCreateMaps" variant="danger" @click="deleteMap">
          {{ _t('Delete map…') }}
        </CmkButton>
...
import { initializeComponentRegistry } from '@/cmk-additions/form/private/FormEditDispatcher/dispatch'
import type {
  ValidationMessage,
...
import { ApiError, mapsApi, mapsApiFormSpec, connectionsApi, rolesApi } from '@/api/client'
...
import { openUrl } from '@/utils/mapNavigation'
...
import { useDebounceFn } from '@cmk/lib/useDebounce'
...
const currentConnectionId = computed(
  () => (formSpecData.value.connection_id as string | undefined) ?? props.map.connection_id
)
provide('orbConnectionId', currentConnectionId)
// Deleting a map and managing its permissions are admin-only; a non-admin
// with edit rights may change settings but not remove or re-share the map.
const tabs = computed<{ id: 'general' | 'permissions'; label: string }[]>(() =>
  auth.isAdmin
    ? [
        { id: 'general', label: _t('Settings') },
        { id: 'permissions', label: _t('Map Permissions') }
      ]
    : [{ id: 'general', label: _t('Settings') }]
)
...
// Optional fields are omitted entirely when the map has no value, so the
...
const rotationInterval = props.map.rotation_interval ?? 0
const formSpecDataInitial: Record<string, unknown> = {
  alias: props.map.alias,
  connection_id: props.map.connection_id,
  rotation_interval: rotationInterval > 0 ? ['every', rotationInterval] : ['off', null],
  click_action: props.map.click_action !== 'none',
  render_mode: props.map.render_mode ?? 'default',
  default_z: props.map.default_z ?? 1,
  show_in_lists: props.map.show_in_lists !== false
}
if (props.map.icon_size != null) formSpecDataInitial.icon_size = props.map.icon_size
if (props.map.hover_template) formSpecDataInitial.hover_template = props.map.hover_template
if (props.map.context_template)
  formSpecDataInitial.context_template = props.map.context_template
...
function openMap() {
  emit('close')
  window.location.hash = `#/maps/${encodeURIComponent(props.map.name)}`
}
...
async function save() {
  ...
  try {
    ...
    if (form.value.map_type === 'flow') {
      ...
      const fresh = await mapsApi.get(props.map.name, auth.accessToken!)
    ...
    const updated = await mapsApi.update(
      props.map.name,
      {
        alias: (fs.alias as string) ?? props.map.alias,
        connection_id: (fs.connection_id as string) ?? props.map.connection_id,
        ...
        default_z: (fs.default_z as number | undefined) ?? props.map.default_z ?? 1,
        ...
        // map metadata never clobbers the slide.
    ...
    toast.success(_t('Map settings saved'), {
      label: _t('Open map'),
      onClick: openMap
    })
...
// Tracks the persisted map version so subsequent saves use the
...
function postPreviewPatch() {
  ...
  // Preview den gleichen geographischen Bereich wie das Eltern-Map zeigt.
  if (
    form.value.map_type === 'worldmap' &&
...
async function confirmDelete() {
  ...
  try {
    await mapsApi.delete(props.map.name, auth.accessToken!)
...
onMounted(async () => {
  const flowSchemaPromise =
    form.value.map_type === 'flow'
      ? mapsApiFormSpec.getFlowViewSchema(auth.accessToken!).catch((): null => null)
  ...
  const [bs, spec, flowSpec] = await Promise.all([
    ...
    mapsApiFormSpec.getMetadataSchema(auth.accessToken!).catch((): null => null),
    flowSchemaPromise,
  ...
  if (spec) {
    // Drop metadata fields that have no effect for the map type: radar and
    ...
    if (form.value.map_type === 'radar') {
      ...
      // The presentation map has no coordinate-positioned icons, no
...
.map-settings__template-preview {
  display: flex;
...
.map-settings__template-preview-label {
  color: var(--text-muted);
}

.map-settings__template-preview code {
  color: var(--text);
...
.map-settings__scroll
  > div
  > .map-settings__type-section
  + :deep(.form-dictionary)
  tr[data-group]
  > td,
.map-settings__scroll :deep(.form-dictionary) tr[data-group] + tr[data-group] > td {
  border-top: 1px solid var(--border);
  padding-top: var(--dimension-5);
  padding-bottom: var(--dimension-5);
}

.map-settings__scroll :deep(.form-dictionary) > tbody > tr[data-group]:first-child > td {
  padding-top: var(--dimension-5);
  padding-bottom: var(--dimension-5);
}

.map-settings__type-section ~ :deep(.form-dictionary) {
  margin-top: 0;
}
...
.map-settings__perm-cmk > * + * {
  margin-top: var(--dimension-6);
}

.map-settings__perm-cmk-intro {
  font-size: var(--font-size-large);
  color: var(--text);
}
