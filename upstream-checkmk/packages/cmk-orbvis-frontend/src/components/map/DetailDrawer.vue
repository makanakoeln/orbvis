...
import type { MapObject, BulkAckTarget, ObjectState } from '@/types/api'
import { buildCheckmkUrl, stripCheckmkBase, summarySubject } from '@/utils/mapNavigation'
import { getMapObjectName, getObjectTypeLabel } from '@/utils/naming'
import { stateColor } from '@/utils/stateColors'
import { formatRelativeDuration, formatRelativeFuture } from '@/utils/time'
import CmkButton from '@cmk/components/CmkButton'
import CmkChip from '@cmk/components/CmkChip.vue'
import CmkCode from '@cmk/components/CmkCode.vue'
import CmkIcon from '@cmk/components/CmkIcon'
import CmkTabs, { CmkTab, CmkTabContent } from '@cmk/components/CmkTabs'
...
const props = defineProps<{
  ...
  /** Map's connection_id — used to fetch on-demand object details. */
  connectionId?: string | null
  /** CSS selector for the StatusSlideIn portal target. Defaults to body. */
  portalTarget?: string
  /** Hostnames currently on the map — topology entries to those hosts
...
const emit = defineEmits<{
  ...
  /** Host name picked from the topology section — map may highlight + select it.
   *  ``seed`` carries a node-derived ObjectState (sans object_id) so drilldowns
   *  into objects without a map state entry still render a status pane. */
  'select-host': [
    hostName: string,
...
const displayName = computed(() => (props.object ? getMapObjectName(props.object) : ''))
