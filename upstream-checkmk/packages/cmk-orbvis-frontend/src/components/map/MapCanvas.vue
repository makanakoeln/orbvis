<template>
  ...
  >
    ...
    <!-- One SVG layer per distinct line-z so lines interleave with the HTML
             ...
             natural proportions when the canvas asymmetric-stretches; MapLine
...
import MapZoomResetPill from './MapZoomResetPill.vue'
...
const props = defineProps<{
  ...
  snapGrid?: number
  /**
   * Search needle from the top-level map search bar. Non-matching objects
   * dim to 25% so the operator's eye snaps to matches without losing the
   * spatial layout — same approach as FlowMap's filter.
   */
  filterNeedle?: string
...
const emit = defineEmits<{
  ...
  'object-dblclick': [obj: MapObjectType]
  'object-delete': [obj: MapObjectType]
  'object-duplicate': [obj: MapObjectType]
  'object-straighten': [obj: MapObjectType]
  'object-detach': [obj: MapObjectType]
  'line-drag-start': [
    event: MouseEvent,
    obj: MapObjectType,
...
const _dragObj = ref<MapObjectType | null>(null)
...
const bgImageUrl = computed(() => {
  ...
  return `${import.meta.env.BASE_URL}maps/backgrounds/${bg}?v=${bgImageCacheKey.value}`
})
...
watch(
  () => mapsStore.bgRefreshTicks[props.config.name],
  (tick) => {
...
const _widthExtent = (o: MapObjectType) =>
  o.x + (o.type === 'graph' ? (o.graph_width ?? 400) : 150)
const _heightExtent = (o: MapObjectType) =>
...
// An object without an explicit z inherits the map's default_z (NagVis-style
// global default), falling back to 1 for maps saved before that field existed.
function resolveZ(obj: MapObjectType): number {
...
const lineLayers = computed(() => {
  const groups = new Map<number, MapObjectType[]>()
...
function objectWrapperStyle(obj: MapObjectType) {
  ...
  const zIndex = _dragId.value === obj.id ? 100 : filterActive.value && matches ? 49 : resolveZ(obj)
...
function matchesSearch(obj: MapObjectType): boolean {
  return (
...
function lineDimStyle(line: MapObjectType) {
...
function onObjectPointerDown(event: PointerEvent, obj: MapObjectType) {
  ...
  const cursorNative = viewportToNative(event.clientX - rect.left, event.clientY - rect.top, rect)
...
function onGraphResizeStart(event: PointerEvent, obj: MapObjectType) {
...
function onLineClick(line: MapObjectType) {
  if (props.editMode) {
...
// in `states`. State resolves from the props map so radar maps keep their
// own filtered map.
...
// feeds <MapObject :icon-size>. Used to anchor bound line endpoints on the
// object's edge rather than its centre.
function objectRenderSize(o: MapObjectType): number {
  return (
...
// Map-coord offset of an object's *visible* icon from its (x, y) anchor, plus
...
function boundCoordsFor(line: MapObjectType): {
  x?: number
  y?: number
  x2?: number
  y2?: number
...
function openSubtreeHover(event: MouseEvent, obj: MapObjectType, state: ObjectState) {
