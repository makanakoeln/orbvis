<!--
  OrbVis-native slide-in shell for the map's status drawer.

  Why this exists instead of using the vendored `CmkSlideIn`:
    - Upstream CmkSlideIn is hardcoded `modal=true` (overlay + body inert),
      which kills map interaction while triaging a host. We want the
      operator to keep clicking around the map with the drawer open.
    - Upstream portals to `body` only; we want the drawer to live inside
      the map canvas container so the topbar/sidebar stay free.
