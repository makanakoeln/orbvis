<template>
  <div class="orb-edit">
    ...
    <div class="orb-edit__header">
      ...
      <div class="orb-edit__header-text">
        ...
        <div class="orb-edit__hint" :class="{ 'orb-edit__hint--placing': placing }">
          {{ placing ? _t('Click on map to place…') : _t('Drag objects · click to select') }}
        </div>
      </div>
      <button
    ...
    <div class="orb-edit__form">
      ...
      <template v-else-if="draft.type === 'aggregation'">
        ...
        <div v-else-if="aggregationPreview" class="orb-edit__aggr-preview">
          ...
          <ul class="orb-edit__aggr-leaves">
            <li v-for="leaf in aggregationPreviewLeaves" :key="leaf.id" class="orb-edit__aggr-leaf">
          ...
          <!--
                        ...
                        worldmap/static maps. Suggest dropping expand_depth
      ...
      >
        {{ placing ? _t('Click on map to place…') : _t('Place on map') }}
      </CmkButton>
...
const mapNames = computed(() => mapsStore.maps.map((b) => b.name))
const mapLabels = computed(() => mapsStore.maps.map((b) => b.alias || b.name))
...
async function fetchAddObjects(type: string) {
  ...
  try {
    addObjects.value = await connectionsApi.objects(props.connectionId, type, auth.accessToken!)
  } catch {
    addObjects.value = []
  } finally {
...
function onTypeChange() {
  ...
  if (props.draft.type === 'map') {
    if (mapsStore.maps.length === 0) mapsStore.fetchMaps()
    return
  }
