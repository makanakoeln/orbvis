"""Presentation board schemas — a design-first, slide-style board type.

The presentation board deliberately departs from the static board's
config-/binding-first model. Instead of a flat ``list[BoardObject]`` whose
...
``BoardConfig.objects`` so the whole slide saves atomically through the existing
...
slide stage (``width × height``), not the static board's percentages.

Kept self-contained on purpose: the module imports nothing from ``board.py`` so
``board.py`` can import ``PresentationView`` for its view union without a cycle,
...
class ElementDisplay(BaseModel):
    ...
    board's ``DisplayConfig`` so the frontend ``GadgetRenderer`` consumes both."""
