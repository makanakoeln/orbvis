...
try:
    from cmk.ccc.site import url_prefix as _url_prefix
except ImportError:
...
def _is_local_install() -> bool:
    ...
    local backend to link to, so registering a main-menu entry would render
    a button that 404s on every click. Use the Apache vhost drop-in (etc/
    apache/conf.d/orbvis.conf) as the install marker since it sits outside
    the replication paths and is only written by install_cmk.sh / orbvis-setup.
    """
