  ...
  <div class="max-w-3xl">
    ...
    <Teleport to="body">
      <div v-if="editRole" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-[var(--bg-surface)] ring-1 ring-[var(--border)] shadow-2xl shadow-black/50 rounded-2xl w-[28rem] max-h-[90vh] flex flex-col overflow-hidden">
          <div class="flex items-center justify-between px-6 pt-5 pb-4 shrink-0 border-b border-[var(--border)]">
          ...
          <div class="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <div v-if="draftPerms.length" class="divide-y divide-[var(--border)] rounded-lg ring-1 ring-[var(--border)] overflow-hidden">
                <div v-for="perm in draftPerms" :key="perm.perm_id"
                  ...
                  <span v-if="perm.perm_id < 0" class="text-[10px] text-indigo-400 shrink-0">new</span>
                  <button @click="removeDraftPerm(perm.perm_id)"
            ...
            <div class="border-t border-[var(--border)] pt-4">
              ...
              <form @submit.prevent="addDraftPerm" class="space-y-3">
                <div class="space-y-1.5">
                ...
                <div v-if="needsMapName" class="space-y-1.5">
                  <label class="text-xs font-medium text-zinc-400">{{ t('admin.boardNameLabel') }}</label>
                ...
                <div class="flex justify-end">
                  <button type="submit" :disabled="!permPreset"
    ...
    for (const perm of draftPerms.value) {
      if (perm.perm_id < 0) {
        ...
        for (const r of roles.value) {
          const p = r.permissions.find(p => p.mod === perm.mod && p.act === perm.act && p.obj === perm.obj)
          if (p) { existing = p; break }
        }
        if (!existing) {
          existing = await rolesApi.createPermission(perm.mod, perm.act, perm.obj, auth.accessToken!)
        }
