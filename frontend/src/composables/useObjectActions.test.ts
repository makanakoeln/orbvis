...
import type { BoardObject, DowntimeEntry, ObjectState } from '@/types/api'
...
function hostObj(over: Partial<BoardObject> = {}): BoardObject {
  return { id: 'o1', type: 'host', host_name: 'web01', ...over } as BoardObject
}
function serviceObj(over: Partial<BoardObject> = {}): BoardObject {
  return {
    id: 'o2',
    type: 'service',
    host_name: 'web01',
...
describe('useObjectActions — command dispatch', () => {
  ...
  it('does nothing for an object without a host name', async () => {
    const { handlers } = useObjectActions(URL)
    await handlers.forceCheck({ id: 'o1', type: 'host' } as BoardObject)
    expect(cmkApi.forceCheckHost).not.toHaveBeenCalled()
    expect(cmkApi.forceCheckService).not.toHaveBeenCalled()
  })
