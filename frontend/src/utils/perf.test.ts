...
describe('parsePerfData', () => {
  ...
  it('parses a single metric', () => {
    ...
    expect(result[0]).toMatchObject({ label: 'rta', value: 1.5, unit: 'ms', warn: 50, crit: 200 });
  });
...
describe('utilPercent', () => {
  ...
  it('uses value directly for percent unit', () => {
    const m = { label: 'x', value: 80, unit: '%', warn: null, crit: null, min: null, max: null };
    expect(utilPercent(m)).toBe(80);
  });

  it('returns 0 when no reference point', () => {
    const m = { label: 'x', value: 50, unit: 'ms', warn: null, crit: null, min: null, max: null };
    expect(utilPercent(m)).toBe(0);
  });

  it('clamps to 100', () => {
    const m = { label: 'x', value: 200, unit: '%', warn: null, crit: null, min: null, max: null };
