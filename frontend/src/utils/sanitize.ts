...
// Single source for every URL sink fed by board data (template hrefs here,
// click navigation in boardNavigation.ts). Mirrors the backend allowlist in
// backend/app/schemas/_validators.py — keep the three in sync. The remote
// access schemes (ssh/rdp/…) are user-mediated OS handlers monitoring boards
