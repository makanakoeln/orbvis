  ...
  <Teleport to="body">
    ...
    >
      ...
      <div class="h-0.5 bg-[var(--border)]">
        <div class="h-full bg-indigo-500 transition-all duration-300" :style="{ width: `${(step / TOTAL) * 100}%` }" />
      </div>
      ...
      <div class="flex items-center justify-between px-5 pb-4">
        ...
        <div class="flex items-center gap-1.5">
          ...
          <!-- Last step non-admin -->
          <button v-else-if="step === TOTAL" @click="finish"
            class="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-sm font-semibold text-white transition-all">
            {{ t('onboarding.finish') }}
          </button>
          <!-- Not last step -->
...
function onResize() { measureTarget() }
...
function skip()        { markDone(); emit('close') }
function finish()      { markDone(); emit('close') }
function createBoard() { markDone(); emit('createBoard') }
</script>
