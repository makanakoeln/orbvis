<template>
  <div class="space-y-2">
    ...
    <div v-if="modelValue" class="flex items-center gap-2 px-3 py-2 bg-[var(--bg-input)] ring-1 ring-indigo-500/50 rounded-lg">
      <img :src="`${BASE_URL}icons/${modelValue}`" class="w-8 h-8 object-contain shrink-0" :class="modelValue.endsWith('.svg') ? 'svg-icon' : ''" />
    ...
    >
      <div v-if="open" class="rounded-lg ring-1 ring-zinc-700 bg-[var(--bg-surface)] overflow-hidden">
        ...
        <!-- Icon grid + upload button at bottom -->
        <template v-else>
          ...
          <div class="px-2 pb-2 border-t border-zinc-700/60 pt-2 flex items-center justify-between">
            <span class="text-[10px] text-zinc-600">{{ icons.length }} icon{{ icons.length === 1 ? '' : 's' }}</span>
...
const filtered = computed(() =>
  query.value
    ? icons.value.filter(i => i.name.toLowerCase().includes(query.value.toLowerCase()))
    : icons.value
)
...
watch(open, (isOpen) => {
  if (isOpen && !icons.value.length) fetchIcons()
})
</script>
