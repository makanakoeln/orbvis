...
function _fmtVal(normalized: number): string {
  return fmtMagnitude(normalized) + (baseUnit(props.unit) ? ' ' + baseUnit(props.unit) : '');
}
...
// Y-axis formatter: normalized values are already in base units, fmtMagnitude applies SI prefix.
// We append the base unit symbol (e.g. "B", "%") as the axis name instead of per-tick.
function _fmtAxis(v: number): string {
  const u = baseUnit(props.unit);
  // For percentage values, append directly so "42.0" becomes "42.0%"
  if (u === '%') return fmtMagnitude(v) + '%';
  return fmtMagnitude(v);
}
...
const option = computed((): EOption => {
    ...
    if (!multiSeries && i === 0 && props.thresholds) {
      ...
      if (props.thresholds.crit !== null) {
        markLineItems.push({
          ...
          lineStyle: { color: 'rgb(248,113,113)', type: 'dashed', width: 1, opacity: 0.75 },
          label: { show: false },
        });
      }
    }
  ...
  return {
    ...
    animation: false,
    grid: { top: 8, right: 6, bottom: 20, left: 38, containLabel: false },
    xAxis: {
      type: 'time',
    ...
    yAxis: {
      type: 'value',
      name: unitLabel && unitLabel !== '%' ? unitLabel : '',
      nameTextStyle: {
        color: textColor,
        fontSize: 8,
        fontFamily: 'ui-monospace,monospace',
