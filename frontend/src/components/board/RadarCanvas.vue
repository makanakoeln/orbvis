<template>
  <div class="flex-1 overflow-auto bg-[var(--bg)] p-6">
    <!-- Empty state -->
    <div v-if="!sortedStates.length"
      class="flex flex-col items-center justify-center h-full text-center">
      <div class="w-14 h-14 rounded-2xl bg-[var(--bg-input)] ring-1 ring-[var(--border)] flex items-center justify-center mb-4">
    ...
    <div v-else class="mb-5 flex items-center gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <span v-for="s in summary" :key="s.state"
    ...
    <div class="grid gap-2.5"
      ...
      >
        ...
        <div class="flex items-start justify-between gap-2 mb-2">
          <span class="font-mono text-xs font-semibold leading-tight break-all"
            :class="nameClass(state.state)">
            {{ displayName(state) }}
          </span>
          <!-- Ack / Downtime icons -->
          <div class="flex gap-1 shrink-0 mt-0.5">
            <span v-if="state.acknowledged" title="Acknowledged"
              class="text-zinc-400 opacity-70">
              <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </span>
            <span v-if="state.in_downtime" title="In downtime"
              class="text-zinc-400 opacity-70">
              <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <!-- State badge -->
        <span class="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md"
        ...
        <!-- Output -->
        <p v-if="state.output" class="text-[11px] mt-2 leading-snug opacity-60 line-clamp-2"
          :class="nameClass(state.state)">
          {{ state.output }}
        </p>
      </div>
...
const sortedStates = computed(() =>
  Object.values(props.states).sort(
