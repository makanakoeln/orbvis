<template>
  <div class="space-y-2">
    ...
    <div v-if="modelValue" class="flex items-center gap-2 px-3 py-2 bg-[var(--bg-input)] ring-1 ring-indigo-500/50 rounded-lg">
      <img :src="`${BASE_URL}images/${modelValue}`" class="w-8 h-8 object-contain shrink-0" :class="modelValue.endsWith('.svg') ? 'svg-icon' : ''" />
      <span class="flex-1 text-xs font-mono text-zinc-300 truncate">{{ modelValue }}</span>
      <button @click="$emit('update:modelValue', '')" class="text-zinc-500 hover:text-zinc-300 transition-colors" type="button">
    ...
    >
      <div v-if="open" class="rounded-lg ring-1 ring-zinc-700 bg-[var(--bg-surface)] overflow-hidden">
        ...
        <!-- Loading -->
        <div v-if="loading" class="flex items-center justify-center py-6 text-zinc-600 text-xs gap-2">
        ...
        <template v-else-if="!images.length">
          <div class="px-4 pt-5 pb-3 text-center">
            <svg class="w-8 h-8 mx-auto mb-2 text-zinc-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
            </svg>
            <p class="text-xs text-zinc-500 mb-3">{{ t('admin.noIcons') }}</p>
            <label class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-xs font-semibold text-white transition-all cursor-pointer">
              <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
              </svg>
              {{ t('boardSettings.uploadIconInline') }}
              <input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" multiple class="hidden" @change="uploadImages" />
            </label>
          </div>
          ...
          >
            <div v-if="uploading || uploadError" class="px-3 pb-3">
              <div v-if="uploading" class="flex items-center gap-2 text-xs text-zinc-400 justify-center py-1">
              ...
              <div v-else-if="uploadError" class="text-xs text-red-400 text-center">{{ uploadError }}</div>
            </div>
          </Transition>
        </template>
        ...
        <template v-else>
          <div class="grid grid-cols-4 gap-1 p-2 max-h-52 overflow-y-auto">
            <button
              ...
              :title="image.name"
            >
              <img :src="`${BASE_URL}${image.url}`" :alt="image.name" class="w-8 h-8 object-contain" :class="image.name.endsWith('.svg') ? 'svg-icon' : ''" />
              <span class="text-[9px] font-mono text-zinc-500 truncate w-full text-center leading-tight">{{ image.name }}</span>
            </button>
          </div>

          <!-- Upload more -->
          <div class="px-2 pb-2 border-t border-zinc-700/60 pt-2 flex items-center justify-between">
            <span class="text-[10px] text-zinc-600">{{ images.length }} icon{{ images.length === 1 ? '' : 's' }}</span>
            <label class="inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700/50 transition-all cursor-pointer">
